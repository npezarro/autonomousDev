...
**Issue:** The Memory Update step hardcodes `~/.claude/projects/-mnt-c-Users-npeza/memory/` as the memory path. Claude Code memory paths are derived from the working directory, so sessions started from `/home/npezarro` (WSL home) use `-home-npezarro` while sessions from `/mnt/c/Users/npeza` use `-mnt-c-Users-npeza`. Any agent following this guidance from the wrong working directory would write to the wrong memory path.
...
**Issue:** `MEMORY_DIR` is hardcoded to `$HOME/.claude/projects/-mnt-c-Users-npeza/memory`. This means the learning agent only scans memory files from Windows-path sessions. Memories saved from Linux-home sessions (`-home-npezarro`) are invisible. S8 (run #3) flagged this; still unfixed after 8 runs.
...
### S32: autonomousDev learnings-12 branch stuck behind main
