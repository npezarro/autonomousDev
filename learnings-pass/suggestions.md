...
## 2026-04-09 — Learning Agent Run #42 (this run)

### S125: PAT scope blocking repos — 293 branches (S122 — 21st escalation, CRITICAL)
**File:** GitHub PAT configuration (infrastructure)
**Issue:** S60→...→S122→still unfixed. Unmerged `claude/auto-*` and `claude/learnings-*` branches total **293** across repos (up from 292 at run #41). The fine-grained PAT only covers a subset of repos, so autonomousDev runs push branches but can't create PRs. First flagged run #25, now 21st consecutive run.
**Suggestion:** Switch to a classic PAT with full repo scope, or add all actively-developed repos to the fine-grained PAT. Then bulk-create PRs for pending branches.
**Priority:** CRITICAL — 21st time flagged. Branch count 292→293. No progress on underlying PAT scope issue.

### S126: Stale branch accumulation — 293 branches (S123 — 22nd flag, HIGH)
**File:** Multiple repos
**Issue:** 293 stale `claude/auto-*` and `claude/learnings-*` branches. The auto-merger never deletes source branches after merge. PAT-blocked branches accumulate indefinitely. First flagged S37 (run #15), now 22nd time flagged.
**Suggestion:** Add post-merge branch deletion to auto-merger. Run periodic cleanup for branches whose content is already on main.
**Priority:** HIGH — 22nd time flagged.

### S127: pm-interview-practice still missing CLAUDE.md (S124 — 15th flag)
**File:** `pm-interview-practice/CLAUDE.md` (does not exist)
