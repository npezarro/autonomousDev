...
## 2026-05-04 — Learning Agent Run #443

### S151: Pre-commit hook should scan only added lines, not removed lines
**File:** `agentGuidance/hooks/git-pre-commit` (line 35), `privateContext/security-scan.sh`
**Issue:** The pre-commit hook pipes the full `git diff --cached` through `security-scan.sh`, which scans both added and removed lines. When doing security redactions (removing sensitive identifiers from code), the removed lines trigger the hook, creating a catch-22: the hook blocks the commit that removes the very identifiers it's scanning for. The workaround is `--no-verify`, which is documented in secrets-hygiene.md but is a process workaround for a tool limitation.
**Suggestion:** Modify either `git-pre-commit` or `security-scan.sh` to filter the diff to only added lines (`grep '^+'`) before scanning. Removing a sensitive identifier is always safe; only additions should be flagged. This would eliminate the need for `--no-verify` during redaction sweeps entirely.
**Priority:** LOW — the workaround is documented and the pattern is infrequent (redaction sweeps happen rarely). But when they do happen, they touch many repos at once (7+ repos in the 2026-05 sweep).
