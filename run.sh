...
if [ -x "$JOURNAL_SCRIPT" ] && [ $EXIT_CODE -eq 0 ] && [ -n "$RESULT" ]; then
  ...
  "$JOURNAL_SCRIPT" "discovery" "autonomousDev run #$RUN_NUMBER: $JOURNAL_SUMMARY" || true
elif [ -x "$JOURNAL_SCRIPT" ] && [ $EXIT_CODE -ne 0 ]; then
  "$JOURNAL_SCRIPT" "blocker" "autonomousDev run #$RUN_NUMBER failed (exit $EXIT_CODE, cost $COST)" || true
fi
